...
Tablero::Tablero(int filas,int columnas){
    int n_fils = filas +1;
    int n_cols = columnas +1;
    Size = n_cols;
    max_cols_allowed = n_cols;
    max_fils_allowed = n_fils;
    ...
    //Inincializamos triángulos
    for(int i = 0; i < n_fils-1; i++)
        for(int j = 0; j < n_cols-1; j++){
            v_triangulos.push_back(_vertex3i(i*n_cols + j,i*n_cols + j +1,i*n_cols +j + n_cols));
            v_triangulos.push_back(_vertex3i(i*n_cols + j +1,i*n_cols +j + n_cols+ 1,i*n_cols + j + n_cols));
        }
